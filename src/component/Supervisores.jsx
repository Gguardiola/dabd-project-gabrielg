...
export default function Supervisores(){
    ...
    const[ingenierosSupervisados, setIngenierosSupervisados] = useState("")
    //get personas
    useEffect(() =>{
        ...
        async function fetchIngenieros(newSupervisor){
            console.log("fetch ingenieros"+newSupervisor)
            try{
                const response = await apiEndpoint.post('/list_ingenieros_supervisados',
                    {
                        auth: "yes",
                        id_viaje_s: newSupervisor,
                    }
                ...
                console.log(response.data.body)
                setIngenierosSupervisados(JSON.parse(response.data.body))
    ...
    const handleDeleteButton = async (e) => {
        const result = window.confirm('¿Deseas confirmar la eliminación de la supervisión sobre '+e.id+' ?');
        if (result) {
            ...
            <Card className='drop-shadow-md	h-full'>
                ...
                <div class="w-full col-span-3 overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full h-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody className="">
                        { ingenierosSupervisados ? (
                            ingenierosSupervisados.map((result) => 
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a onClick={() => handleDeleteButton({id: result.id_viaje_i})} class="font-medium text-red-600 dark:text-red-500 cursor-pointer">Borrar</a>
                                </td>
                            </tr>
                            )) 
                            : (
